...
import { UserIcon, ShareIcon, HelpIcon, CheckIcon, BackIcon, CloseIcon, NewChat } from '../../../elements/icons/MaxSoloIcons';

  const SideBar = ({ restartChatContacts, getPublicKey, contacts, lastMessage, restartContacts, responseContact, responseName }) => {
      ...
      <div className={`maxsolo-sidebar ${mobile ? 'mobile' : ''}`}>
        <div className="maxsolo-sidebar-header">
          <div className="header-detail">
            ...
            <div onClick={onChatContactsClick} className={`header-options ${mobile ? 'hide' : ''}`}>New chat <NewChat /></div>
          </div>
        </div>
        ...
        <AddContact setSelectedTab={setSelectedTab} setIsOpen={setIsOpen} isSelectedTab={isSelectedTab} restartChatContacts={restartChatContacts}/>
        
        <div className={`maxsolo-sidebar-menu ${isOpen ? 'open' : ''}`}>

          <div className='close-window' onClick={onCloseClick}><CloseIcon /></div>
        ...
        ? (<>
            <div className='maxsolo-sidebar-contacts'>
              ...
              {lastMessage.filter((item) =>
                item.ROOMNAME
                  .toLowerCase()
                  .includes(inputSearch.toLowerCase())
                  ).map(((item, index)=>(
                    <div key={index} className={`contact ${contacts.filter(data => data.publickey === item.PUBLICKEY).map(data => (Boolean((Math.abs(new Date() - data.lastseen))/60000 < 30)))} ${active === item.PUBLICKEY && 'active'}`} onClick={() => SideBarHandler(item.PUBLICKEY, item.ROOMNAME)}>
                      <Avatar className="contact-profile" name={item.ROOMNAME} size={54} round={true} maxInitials={2}/>
                      <div className="contact-detail">
                        <div className="contact-username">{item.ROOMNAME}</div><span className="contact-date">{getTime(item.DATE)}</span>
