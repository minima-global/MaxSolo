...
const ChatArea = ({
  ...
  const uploadImage = async (event) => {
    try {
      ...
      if (!file.type.match(imageMimeType)) {
        notifyRef.current.notifyMessage(
          "Image mime type is not valid, only images supported.",
          "error"
        );
        return;
      }
  ...
  const checkVault = (e) => {
    ...
    if (tokenamountsend.greaterThan(tokenamountsendable))
      return onSetErrorClick("Insufficient funds...");
  ...
  const submitToken = () => {
    ...
    const roundedAmount = new Decimal(tokenamount)
      .toDecimalPlaces(18)
      .toString();
    const tokenamountsend = new Decimal(inputToken);
    const tokenamountsendable = new Decimal(tokenSendable);
    ...
    window.MDS.cmd(
      "maxcontacts action:search publickey:" + publicRoomKey,
      function (resp) {
        //Get the contact
        const sendfunction =
          vaultValue == ""
            ? `send tokenid:${tokenid} amount:${tokenamountsend} address:${getMinimaAddress}`
            : `send password:${vaultValue} tokenid:${tokenid} amount:${tokenamountsend} address:${getMinimaAddress}`;
        ...
        window.MDS.cmd(sendfunction, function (resp) {
          ...
          const tokenSendData = (message) => {
            var data = {};
            data.message =
              "" + message + " " + roundedAmount + " " + tokenname + "";
  ...
  const deleteMessages = () => {
    ...
    window.MDS.sql(
      "DELETE from messages WHERE publickey='" + publicRoomKey + "'",
      function (sqlmsg) {
        if (sqlmsg.status) {
          loadMessages();
          restartContacts();
  ...
  const deleteContact = () => {
    const deletecontact =
      "maxcontacts action:remove contact:" +
      getCurrentAddress +
      " id:" +
      getContactId +
      "";
    ...
    <>
      ...
      <div className="maxsolo-chat-area">
        <div className="maxsolo-chat-area-header">
          <div
            className="maxsolo-chat-area-title"
            onClick={handleUserProfileClick}
          >
            <Avatar
              className="maxsolo-chat-area-title-profile"
              name={roomName}
              size={54}
              round={true}
              maxInitials={2}
            ...
            <div className="maxsolo-chat-area-title-room">
              {roomName}
              <span className="maxsolo-chat-area-title-room-lastseen">
                Last seen {lastSeen}
              </span>
            </div>
          </div>
          <div className="maxsolo-chat-area-group">
            <DropDown
              deleteMessages={deleteMessages}
              getMaximaContact={getMaximaContact}
              roomName={roomName}
            />
          </div>
        </div>
        ...
        <div
          className={`send-tokens-window ${showToken ? "" : "hide"} ${
            active ? "height" : ""
          }`}
        >
          ...
          <div className="send-tokens-window-header">
            {active ? "Amount to send" : "Select token"}
          </div>
          <div className={`send-tokens-window-number ${active ? "" : "hide"}`}>
            <input
              ...
              value={inputToken}
              onInput={(e) => setInputToken(e.target.value)}
              onChange={(event) => {
                try {
                  ...
                  if (event.target.value.length <= 12) {
                    ...
                    setNewSize(
                      parseInt(70 - event.target.value.length * 2.215)
                    );
                  }
                  if (tokenamountsend.greaterThan(tokenamountsendable))
                    onSetErrorClick("Insufficient funds...");
                } catch (err) {
                  // console.log(err);
                }
              }}
          ...
          <div
            ...
            className={`send-tokens-window-max ${active ? "" : "hide"}`}
          >
            MAX
          </div>
          <div className="send-tokens-window-balance">
            {getBalance.map((item, index) => (
              <div
                className={`send-tokens-window-balance-item ${
                  active
                    ? active === tokenName(item.token.name) && "active"
                    : ""
                }`}
                key={index}
                onClick={() =>
              ...
              >
                <div className="token-logotype">
                  <img src={tokenUrl(item.token.url)} alt="" />
                </div>
        ...
        <div
          onClick={onMenuClick}
          className={`chat-area-footer-overlay ${
            showMenu || showToken ? "show" : ""
          }`}
        ></div>
        ...
        <div className="maxsolo-chat-area-main">
          <div className="maxsolo-chat-area-main-notification">
            ...
            <span>
              Messages to this chat with {roomName} are now secured with
          ...
          {/* {chatData.map((item, index) => (
            <div key={index}>
              <div
                className={`chat-msg ${
                  item.ROOMNAME === item.USERNAME ? "" : "owner"
                }`}
              >
                <div className="chat-msg-profile">
                  <div className="chat-msg-date">{getTime(item.DATE)}</div>
                </div>
                <div className="chat-msg-content">
                  <div className="chat-msg-text">
                    {item.FILEDATA ? (
                    ...
                    {item.MESSAGE ? (
                      <span>
                        {linkOutput(
                          decodeURIComponent(item.MESSAGE).replace("%27", "'")
                        )}
                      </span>
                    ) : (
                      ""
            ...
            >
              {baseImage ? (
                <div className="image-preview">
                  <img src={baseImage} height="32px" width="32px" alt="" />
                </div>
              ) : (
                ""
              )}
              ...
              {roomName ? (
                <TextareaAutosize
                  maxRows={messageData.copy ? "5" : "1"}
                  data-testid="input-text"
                  onChange={textareaData}
                  placeholder="Write a message"
                  value={messageData.copy || ""}
                />
              ) : null}
